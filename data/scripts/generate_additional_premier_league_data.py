...
OUTPUT_CSV = os.path.join(os.path.dirname(__file__), '../additional_data/additional_premier_league.csv')
...
def generate_synthetic_rows(df, n_rows):
    ...
    for i in range(n_rows):
        ...
        row['Date'] = (date_min + timedelta(days=int(rand_days))).strftime('%m/%d/%Y %H:%M')
        # Referee
        ...
        # Possession (0-100)
        row['Possession'] = np.clip(np.random.normal(df['Possession'].mean(), df['Possession'].std()), 30, 70)
        # Numeric columns
        for col in numeric_cols:
            ...
            elif 'Points' in col or 'Pts' in col or 'Goals' in col or 'Conceded' in col or 'Shots' in col or 'Fouls' in col or 'Corners' in col or 'Yellow' in col or 'Red' in col:
