...
def _train_model(
    ...
    # Strict no-current-match-leakage feature regime:
    # only pre-declared rolling + categorical columns (no dynamic expansion).
    numeric_cols = BASE_NUMERIC_FEATURES.copy()
    cat_cols = BASE_CATEGORICAL_FEATURES.copy()
    all_cols = numeric_cols + cat_cols
...
def predict_selected_match(selected_option: str):
    ...
    if pred_label == "w":
        winner_text = str(row["Team"])
    elif pred_label == "l":
        winner_text = str(row["Opponent"])
    else:
        winner_text = "DRAW"
    ...
    summary_md = (
        f"# **{winner_text}**\n"
        f"### Predicted outcome: `{pred_label}`\n"
        f"- Confidence: **{top_prob:.2%}**\n"
        f"- Fixture: **{row['Team']} vs {row['Opponent']}**\n"
        f"- Date: **{match_date}**"
    )
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                gr.Markdown(
                    ...
                    f"- Dynamic feature expansion: **OFF**\n"
                    f"- Current-match leakage mode: **OFF**\n"
