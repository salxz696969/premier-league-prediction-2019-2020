...
- `notebooks/v2.ipynb`: current baseline modeling and reports.
...
## 8) Baseline results (from `notebooks/v2.ipynb`)

Reference run currently used by docs (final held-out test):
...
- `Cross validation off` (baseline alignment to `v2.ipynb`)
- `Full` (CV enabled)
- `With leakage true`
- `Additional data off`
